...
A monorepo template for API, web, mobile, and AI-assisted development. Includes a Hono API, TanStack Start web app, Expo mobile app, and a shared functional core for types, schemas, and domain logic. Configured for strict TypeScript, zero-warning linting, automated verification, and AI agent setup.
...
`.ruler/AGENTS.md` is the source of truth for agent setup. Run `pnpm exec ruler apply` to generate configs for Claude Code, Codex CLI, Cursor, Copilot, Gemini CLI, and OpenCode.
...
The repo ships shared skills through the `skills` CLI, covering design, prompt engineering, PR workflow, Expo, and React Native guidance.
...
- Keep the E2E suite paired with a reachable API for passing `/notes` browser flows
